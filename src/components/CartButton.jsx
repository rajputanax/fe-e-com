...
const CartButton = ({handle}) => {
    ...
    button.addEventListener('click', (e) => {
      ...
      gsap.to(button, {
        keyframes: [
          ...
          { '--cart-tick-offset': '0px', '--cart-y': '0px', duration: 0.2, onComplete: () => { button.style.overflow = 'hidden'; } },
          { '--cart-x': '52px', '--cart-rotate': '-15deg', duration: 0.2 },
          { '--cart-x': '104px', '--cart-rotate': '0deg', duration: 0.2, clearProps: true, onComplete: () => {
              button.style.overflow = 'hidden';
              button.style.setProperty('--text-o', 0);
              button.style.setProperty('--text-x', '0px');
              button.style.setProperty('--cart-x', '-104px');
          }},
          { '--text-o': 1, '--text-x': '12px', '--cart-x': '-48px', '--cart-scale': 0.75, duration: 0.25, clearProps: true, onComplete: () => {
      ...
      gsap.to(shirt, {
        keyframes: [
          { morphSVG: 'M4.99997 3L8.99997 1.5C8.99997 1.5 10.6901 3 12 3C13.3098 3 15 1.5 15 1.5L19 3L23.5 8L20.5 11L19 9.5L18 22.5C18 22.5 14 21.5 12 21.5C10 21.5 5.99997 22.5 5.99997 22.5L4.99997 9.5L3.5 11L0.5 8L4.99997 3Z', duration: 0.25, delay: 0.25 },
          { morphSVG: 'M4.99997 3L8.99997 1.5C8.99997 1.5 10.6901 3 12 3C13.3098 3 15 1.5 15 1.5L19 3L23.5 8L20.5 11L19 9.5L18.5 22.5C18.5 22.5 13.5 22.5 12 22.5C10.5 22.5 5.5 22.5 5.5 22.5L4.99997 9.5L3.5 11L0.5 8L4.99997 3Z', duration: 0.85, ease: 'elastic.out(1, .5)' },
  ...
  return (
    <div className="white">
      <button ref={buttonRef} className="add-to-cart" onClick={handle} >
        <span>Add to cart</span>
        <svg className="morph" viewBox="0 0 64 13">
          <path d="M0 12C6 12 17 12 32 12C47.9024 12 58 12 64 12V13H0V12Z" />
        </svg>
        <div className="shirt">
          {/* First & Second shirt SVGs */}
        </div>
        <div className="cart">
          {/* Cart SVG */}
          <AiOutlineShoppingCart size={20}   style={{ marginRight: "8px", color: "white" }}  className="cart-icon"/>
        </div>
      </button>
    </div>
  );
